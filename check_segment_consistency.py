...
from gap_builder import build_gap_object, format_gap_for_prompt
...
def call_ollama(model: str, system: str, user: str, temperature: float = 0.1) -> str:
    """Call Ollama and return raw text. Low temperature for consistency."""
...
def extract_segments_for_file(
    ...
    try:
        ...
        return {"segments": None, "schema_errors": ["llm_call_failed"], "error": str(e), "raw": ""}

    if parsed is None:
        return {"segments": None, "schema_errors": ["json_parse_failed"], "error": parse_error, "raw": raw[:300]}

    schema_errors = validate_schema(parsed)
    return {"segments": parsed, "schema_errors": schema_errors, "error": None, "raw": raw}
...
def process_silver_batch(
    ...
    """For every row in silver_diff.jsonl:
      ...
      - extract segments for both via Ollama
      - compute gap_object via gap_builder
    ...
    for i, row in enumerate(rows, 1):
        ...
        if not fail_path.exists():
            print(f"[{i}/{len(rows)}] MISSING failing file: {fail_path}")
            errors += 1
            continue
        if not pass_path.exists():
            print(f"[{i}/{len(rows)}] MISSING passing file: {pass_path}")
            errors += 1
            continue
        ...
        # ── Build gap object (only if both segments valid) ────────────────────
        gap_object: dict | None = None
        gap_prompt_block: str | None = None
        if (
            fail_result["segments"] is not None
            and not fail_result["schema_errors"]
            and pass_result["segments"] is not None
            and not pass_result["schema_errors"]
        ):
            codebert_sim = row.get("embedding_similarity", 0.5)
            gap_object = build_gap_object(
                fail_result["segments"],
                pass_result["segments"],
                codebert_sim,
            )
            gap_prompt_block = format_gap_for_prompt(gap_object)
